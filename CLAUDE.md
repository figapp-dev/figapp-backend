...
7. FigChat send — **done**. Calendar (`/calendar/events`, `/calendar/eligible-participants`) — **done**, full web parity (recurrence, reminders, participant invites, RSVP, edit-scope). Documents (`GET /documents`, `GET /documents/:id`, `POST /documents/:id/sign` + `documents` bucket on signed-url) — **done** for foster_carer assignee list/preview/sign (web-parity declaration; no drawn pad; upload deferred — admin-only on web Documents page). Note: "Tasks" (`TaskManagement.tsx` on web) is `super_admin`/`app_admin`-only and is **not** a foster_carer feature — don't build a `/tasks` endpoint for this app. Remaining, in the web sidebar's foster_carer order: My Household, Life Story, My Placements, Platform Support (tickets), Survey Center, Expense Claims, Notifications (deliberately last) — one resource at a time, matching Flutter's build order so neither side races ahead.
